...
def _request_distinct(claude: ClaudeClient, blob: str, skill_content: str) -> list[dict]:
    ...
    user_payload = {
        ...
        "output_schema": [
            {
                "emoji": "string - 💰(『ナイルの財布が新たに開く』×『顧客pull(顧客が自分の言葉で欲しがる)』の両方が揃う時のみ。AI推測の提案余地・既存予算内の最適化・既存契約内のサービス・継続維持は💰にせず●) / 🚨(競争・離反/継続リスク) / ●(普通のタスク・情報) のいずれか1つ",
        ...
        "rules": [
            ...
            "★💰ハードゲート＝次の2つが両方揃う時だけ💰、片方でも欠けたら●：(1)ナイルの財布が新たに開く（新規発注/予算増額/今やってない新領域への投資）(2)顧客が自分の言葉でそれを欲しがっている（顧客pull）。次は💰にしない（→●、継続交渉は🚨）：『AIが提案余地/刺さる/提案できそうと推測しただけで顧客pullが無い』『既存契約内のサービス遂行』『既存予算内の最適化・配分替え』『他社/関連会社が担当の作業』『継続契約の維持/死守』『確保済み・決裁者承認待ちの既存予算』。『再提案』『提案余地』『予算確保済み』を💰の根拠にしない。例:顧客が『Web/SNSも1社に任せたい』と明示=💰／年間予算確定下のリライト最適化=●／UI/UXコンサルの提案余地(顧客発注要望なし)=●／契約内メルマガ改善のヒアリング=●",
