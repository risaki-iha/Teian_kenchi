        ...
        "output_schema": [
            {
                ...
                "label": "string - 20〜30字程度の見出し（【】記号なし）。単体の1行で通知の親メッセージに出るため、テーマ名だけでなく『何を・なぜ』に加え**今のフェーズ・次に必要なアクション**まで一目で伝わる具体性を持たせ、『〜の余地あり』『〜が必要』『〜を顧客が希望』のように言い切る（例：「法人保険コンテンツSEO・KW戦略の本格発注に向け提案資料が必要」「EC・CDP・BI統合基盤構築、人員不足でナイルへ体制構築提案の余地あり」）",
                ...
                "is_noise": "boolean - 採用しない項目の場合 true（出力配列から除外）。💰/🚨は『ナイルが動ける余地』『顧客の明確な要望』のどちらも元データに無ければtrue（●は対象外）",
            }
        ],
        "rules": [
            "★最重要: 同じ機会/リスク/タスクの重複は必ず1件に統合する（過分割を出さない）",
            "絵文字は3種類のみ・1項目1絵文字: 💰(アップセル機会＝提案でお金になりそう) / 🚨(競争・離反リスク＝取られる/失う) / ●(それ以外の拾うべきタスク・情報)",
            "★💰ハードゲート＝次の3つが【すべて】揃う時だけ💰、1つでも欠けたら●：(1)ナイルの財布が新たに開く（新規発注/予算増額/今やってない新領域への投資）(2)顧客pull＝顧客本人が自分の口で発注方向に踏み込む（発注/依頼/見積もり/金額/契約/『いつ始める』に言及、または新領域を『やってほしい/欲しい』と明示）(3)ナイルの提供領域（SEO/広告/コンテンツ/サイト改善/計測）の中にある。★顧客pullでないもの（→●）：『困ってる/不便/やらなきゃ/手が回らない/知見がない/できてない』等の課題・不満・リソース不足の『共有』だけ／コンサルが無償の範囲で助言・運用手順を教えた場面を商機に膨らませたもの／発言者がナイル社員・制作パートナー・代理店側（顧客本人でない）。『〜もやりますよね?』『ナイル側で〜』を顧客ニーズと誤読しない。エンド顧客がほぼ発言しない内部/制作すり合わせ会議は慎重に●。★スコープ外（→●）：CMS/システム開発・社内システム改修などナイルが受注しない領域／顧客が『社内・別ベンダー・制作会社に聞く/相談する』と言う＝解決先がナイル外の話。★amptalkネイティブ提案機会minute（『〜の余地あり』『商機が高い』）はAI推測であり顧客pullの証拠にしない（顧客本人の発注志向の発言を必ず確認）。従来通り、AI推測(提案余地/刺さる/提案できそう)だけ・既存契約内のサービス遂行・既存予算内の最適化/配分替え・他社/関連会社担当・継続維持/死守(継続交渉は🚨)・確保済み/決裁者承認待ちの既存予算は💰にしない（『再提案』『提案余地』『予算確保済み』を根拠にしない）。例:顧客『Web/SNSも1社に任せたい』明示=💰／ウォンテッドリー『7/20週着手希望・来週社内決裁』=💰／エールコネクト『8月開始・新規予算100-200万確保』=💰／フラグシップ『SNS拡大』(発言者がナイル/制作側・エンド顧客不在)=●／光文社『WordPress再公開機能開発』(解決先が社内/CMSベンダー=スコープ外)=●／光文社『定期SEO診断』(SearchConsole運用手順を無償助言した場面・顧客は自分で対応)=●／年間予算確定下のリライト最適化=●／UI/UXコンサルの提案余地(顧客発注要望なし)=●",
            ...
            "★💰/🚨限定の採用フィルター(2026-07-23追加・最重要、●は対象外)：次の【どちらか】が元データに無ければis_noise=true。(1)ナイル側が具体的に提案・支援できる余地がある（何をすべきか・何を持ちかけられるかが読み取れる）(2)顧客側から明確な要望・依頼がある。単なる状況共有・進捗報告(『順調』『懸念がある』で終わり対応方針の言及が無い)は拾わない。★(1)は▼メモの『今後の進め方』『対応方針』等のサブセクションに書かれていることが多く見落としやすい最頻出の失敗パターン。▼メモの隅々まで確認し、言及があれば確実に拾ってsummaryに織り込むこと",
            ...
            "label は全項目に付与。20〜30字程度で、『何の機会/リスク/タスクか』だけでなく主語・対象・狙い、さらに**今のフェーズ・次に必要なアクション**まで一目で分かる具体性を持たせる（💰/🚨は親メッセージにこの1行だけが単独表示されるため、label単体で内容が伝わる必要がある）。『〜の余地あり』『〜が必要』『〜を顧客が希望』のように言い切る。【】記号は付けない",
            "summary は『読んだだけで状況が分かる』情報量で書く（80〜120字目安・必要なら2文）。①何が起きたか（状況・経緯）②なぜ機会/リスク/タスクなのか（理由・背景）③だから何が必要か、が伝わること。★③は▼メモの『今後の進め方』等に対応方針の記述がないか必ず確認し、あれば省略せず織り込む（状況説明だけで終わらせない）。★grounding厳守＝書き起こし・要約minuteに実際に話されている事実だけを書き、原文に無い結論/予測/『余地』/金額/『追加投資』『拡大』等を創作しない（例：『予算確保済み』を『追加投資・拡大の余地』と読み替えるのは禁止。確保済み＝既存の金）。②③の理由・含意も元データに根拠がある範囲だけ。例『コンペ再提案』だけでなく『代理店見直しでRFPが発行され…勝てないと失注リスク』のように背景まで書く（ただし元データにある範囲で）。抽象動詞(対応・検討・確認)を避け具体の数字・固有名詞を含める。無理な短縮・略語は禁止。期日表記は含めない（別途【期日】行で表示）",
