...
def _request_distinct(claude: ClaudeClient, blob: str, skill_content: str) -> list[dict]:
    ...
    user_payload = {
        ...
        "output_schema": [
            {
                "emoji": "string - 💰(新規/追加の予算が動くアップセル機会のみ。既存予算の配分替え・継続維持は除外) / 🚨(競争・離反/継続リスク) / ●(普通のタスク・情報) のいずれか1つ",
        ...
        "rules": [
            ...
            "★💰 は『新規／追加の予算が動くアップセル機会』だけに付与（新規発注/予算増額/新規領域への投資/追加提案/次フェーズ/新ソリューション＝新たに財布が開く時）。『既存予算の配分替え』『継続契約の維持・死守/2期目継続交渉』『確保済み・決裁者承認待ちの既存予算』は💰にしない（継続交渉なら🚨、ただのタスクなら●）。『予算確保済み』を💰の根拠にしない。金額小さい・高額難・進行管理・事務TODOも ●",
